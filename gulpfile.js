...
//gulp.task("deploy", gulp.series('jekyll', 'sass', function() {
	return cp.spawn('git status && git commit -am "fix responsive issues, update text in about section, block temporary not working links, GHP deploy" && git pull && git push', { stdio: "inherit", shell: true });
}));